...
JNIEXPORT jint JNICALL Java_server_MLManager_testLocal
  (JNIEnv * env, jobject thisObject, jstring taskName, jobjectArray data, jobjectArray labels) {
    ...
    if (PyCallable_Check(pFunc)) {
        ...
        int * label_batch;
        try {
            batch = convertSamplesToIntTensor(env, data, &batch_size, &img_height, &img_width);  // new
            label_batch = convertLabelsToIntTensor(env, labels, batch_size);  // new
        } catch (...) {
            returnValue = -2;
        }
        if (!returnValue) {
...
(JNIEnv * env, jobject thisObject, jstring taskName, jstring dataPath, jstring dataType, jint batchSize) {
    ...
    pName = PyString_FromString("py_test_remote");
    pModule = PyImport_Import(pName);
    pDict = PyModule_GetDict(pModule);
    pFunc = PyDict_GetItemString(pDict, "test");
    ...
    if (PyCallable_Check(pFunc)) {
        ...
        pValue = PyString_FromString(env->GetStringUTFChars(taskName, NULL));
        PyTuple_SetItem(pArgs, 0, pValue);
        pValue = PyString_FromString(env->GetStringUTFChars(dataPath, NULL));
        PyTuple_SetItem(pArgs, 1, pValue);
        pValue = PyString_FromString(env->GetStringUTFChars(dataType, NULL));
        PyTuple_SetItem(pArgs, 2, pValue);
        pValue = PyInt_FromLong((long) batchSize);
        PyTuple_SetItem(pArgs, 3, pValue);

        pValue = PyObject_CallObject(pFunc, pArgs);
        returnValue = (float) PyFloat_AsDouble(pValue);
